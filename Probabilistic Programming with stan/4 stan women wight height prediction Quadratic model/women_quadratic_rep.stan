...
model{
  ...
  #THe model with generated quantities
  #To see how the model is been able to reproduce its own observation which it has seen before and with what accuracy
  
generated quantities{
   real y_rep[N]; // The replicated data
             // women height y will be real number with N length
    // Check each individual of data to replicate of data step by step
   
   for( i in 1:N){
     y_rep[i] = normal_rng(alpha*x[i] + gamma*x_sq[i]+beta, sigma); //normal distribution same but in different block so with diffrent name
   }
